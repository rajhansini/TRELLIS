...
solve_align_obj.py
------------------
solve_mesh_alignment.py for an ARBITRARY object. The original is untouched.

Two hard-coded paths become arguments:
    --slat-npz    the frozen per-frame SLaT cache (build_slat_cache_obj.py)
    --frames-dir  that object's GT video frames

Everything else -- the two-stage solve, the IoU objective, the stopping rule --
is the original's, unchanged, so an alignment produced here is comparable to the
teapot's (which scored IoU 0.7596 -> 0.9051).

N_vox is NOT 7301 for a new object; it comes from the npz and nothing here
assumes the teapot's value.
...
ap.add_argument('--slat-npz', default=None, type=Path,
                help='frozen SLaT cache for THIS object. Defaults to the --run '
                     'directory, which is the teapot.')
ap.add_argument('--frames-dir', default=None,
                help="that object's GT frames. Defaults to the teapot's.")
args = ap.parse_args()

RUN_DIR  = _LORA / 'runs' / args.run
SLAT_NPZ = Path(args.slat_npz) if args.slat_npz else RUN_DIR / 'slat_cache.npz'
GT_DIR   = Path(args.frames_dir) if args.frames_dir else Path(
                '/net/projects/ranalab/rajhansini/MV-Adapter-Experimental'
                '/outputs/teapot_lava_kling_premium'
                '/teapot_lava_kling_premium_front/all_frames_150')
assert SLAT_NPZ.exists(), f'no SLaT cache at {SLAT_NPZ}'
assert GT_DIR.is_dir(),   f'no frames dir at {GT_DIR}'
