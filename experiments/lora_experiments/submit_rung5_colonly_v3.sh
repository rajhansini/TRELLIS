#!/bin/bash
# Submit rung5 color-only v3 (MLP-only LoRA) decoder training.
# Usage: bash submit_rung5_colonly_v3.sh [--rank R] [--epochs E] [--seed S] [--lambda-reg L]

set -e

PY=/net/projects/ranalab/rajhansini/conda_envs/trellis/bin/python
SCRIPT=/net/projects/ranalab/rajhansini/TRELLIS/experiments/lora_experiments/rung5_colonly_lora_v3.py
...
echo "=== Rung 5 Color-Only Decoder LoRA v3 (MLP-only) ==="
...
TRAIN_JOB=$(sbatch \
    ...
    --output="${LOGDIR}/rung5_mlponly_%j.log" \
    --error="${LOGDIR}/rung5_mlponly_%j.log" \
    --wrap="
set -e
...
echo "  tail -f ${LOGDIR}/rung5_mlponly_${TRAIN_JOB}.log"
