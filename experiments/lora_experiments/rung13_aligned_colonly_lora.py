"""
rung13_aligned_colonly_lora.py
------------------------------
rung5_colonly_lora.py + MESH ALIGNMENT. Copied from its parent; the parent is
untouched and still runs.

WHY — measured, not assumed

  In the training view, over 15 frames:

      GT teapot      centroid x 265.1   width 319   area 30,961
      frozen render  centroid x 248.9   width 355   area 34,483

  The render is 11.2% WIDER, the same height, and 16 px to the side. Every pixel
  of the loss, in every rung so far, has compared two misregistered objects and
  called the difference "texture error".

  solve_mesh_alignment.py (job 2148935) solved a 3D similarity on SILHOUETTE
  OVERLAP ONLY — it never sees colour, so using it to fix a colour artifact is
  not circular:

      s = 0.8903   rotvec = [-0.1091, 0.0015, -0.0530]  (7.0 deg)
      t = [0.0412, -0.0472, 0.0013]
      silhouette IoU 0.7596 -> 0.9051 on 15 frames it was not fitted on
      60.5% of the mismatch was POSE. The residual 0.0949 is genuine shape
      difference and no transform fixes it.

  test_alignment_compensation.py (job 2148951) then confirmed the consequence,
  against thresholds fixed before it ran (>=8 px confirm, <=3 px refute):

      colonly texture on the UNALIGNED mesh : NCC peak dx = 0, 0, 0
      colonly texture on the ALIGNED   mesh : NCC peak dx = +19, +5, +8
      mean movement +10.7 px  ->  CONFIRMED

  The adapter had baked a compensating shift into the texture. It could not move
  the mesh, so it slid the paint until the lava landed on the right screen
  pixels. From the training camera that looks like success; from any other angle
  the texture is in the wrong place. That is the "shifted sticker".

THE ONE CHANGE
  Every render applies the solved transform to the mesh vertices first:

      v' = s * R(rotvec) @ (v - centre) + centre + t

  Applied inside render_mesh(), the single choke point every call site goes
  through, so training, eval, diagnostics and gates all see the same geometry.
  The transform is FIXED — loaded from alignment.json, never learned.

A NECESSARY CONSEQUENCE, NOT A SECOND VARIABLE
  MASK_PATH/mask.png is a cached silhouette of the UNALIGNED render. Once the
  render moves, that file is stale and the union region it defines is wrong. So
  the static render mask is recomputed here from the ALIGNED frozen mesh. Both
  are logged, and PSNR is reported on both, because PSNR on the new mask is NOT
  comparable to the parent's 21.281 — it is a different region.

DELIBERATELY NOT CHANGED
  Union loss and the leaky (gt < 0.99) gt_mask stay exactly as the parent has
  them. Both are known-bad and both are fixed in rung8_intersection_lora.py, but
  changing them here would put two variables in one run. One change at a time.
  No visibility mask either — that is rung11's variable.
...
Usage:
  ...
  python experiments/lora_experiments/rung13_aligned_colonly_lora.py \
...
# ── THE CHANGE: the solved mesh alignment ────────────────────────────────────
...
if not _ALIGN_PATH.exists():
    raise FileNotFoundError(
        ...
        f'run visibility/solve_mesh_alignment.py first.')
...
_LABEL = f'rung13_aligned_r{args.rank}_s{args.seed}_{RUN_ID}'
_OUT   = _RUNS / _LABEL
...
def colonly_forward(dec_model, registry, slat_norm, with_grad=True):
    ...
    Mix: cat([frozen_geom.detach(), lora_col.clamp(ABS_MIN, ABS_MAX)])
...
def render_mesh(mesh, renderer, aligned=True):
    """
    THE RUNG-13 CHANGE. Every render goes through here, so training, eval,
    diagnostics and gates all see the same aligned geometry. aligned=False
    exists only so the gates can measure the before/after difference.
    """
...
def main():
    ...
    # ── mask.png is a cached silhouette of the UNALIGNED render. Once the mesh
    #    moves that file is stale and the union region it defines is wrong, so it
    #    is recomputed here from the ALIGNED frozen mesh. Not a second variable —
    #    a necessary consequence of the first. Both are kept and both reported.
    render_mask_stale = load_render_mask()
    _slat75 = _slat_from_cache(slat_cache, 75)
    with torch.no_grad():
        _mfz = dec_model(_slat75)[0]
    _, _m_al = render_mesh(_mfz, renderer, aligned=True)
    render_mask = (_m_al.squeeze(0) > 0.5)
    _iou_masks = float((render_mask & render_mask_stale).sum()
                       / max(int((render_mask | render_mask_stale).sum()), 1))
    print(f'\n[MASK] static render mask recomputed from the ALIGNED frozen mesh')
    print(f'  stale mask.png : {int(render_mask_stale.sum()):,} px')
    print(f'  aligned mask   : {int(render_mask.sum()):,} px')
    print(f'  IoU(stale, aligned) = {_iou_masks:.4f}   '
          f'(low is EXPECTED — that is the misregistration)', flush=True)

    # ── GATE-align: the transform must actually be applied, and must improve
    #    silhouette agreement with the GT. If it does not, rung13 is rung5 with a
    #    different name and every number below would be meaningless.
    # NOTE: load_gt() returns the parent's LEAKY mask, (gt < 0.99).any(), which
    # fires on the off-white background and covers 99.7% of the image. IoU
    # against that is just |render| / |image| (~0.13) and it PENALISES the
    # aligned render for being smaller — it reads the improvement backwards.
    # Every 0.76 / 0.905 figure in this project came from the TIGHT teapot mask,
    # so the gate computes that directly. This is diagnostics only: the training
    # loss still uses the parent's leaky mask, which is the deliberate
    # "one variable at a time" decision documented at the top of this file.
    _img75 = Image.open(GT_FRAMES_DIR / 'frame_0075.png').convert('RGB') \
                  .resize((RENDER_RES, RENDER_RES), Image.LANCZOS)
    _a75   = torch.from_numpy(np.array(_img75)).float().div(255.0).permute(2, 0, 1).to(DEVICE)
    _gm75  = (_a75.min(dim=0).values < 0.95)
    print(f'  [tight GT mask] {int(_gm75.sum()):,} px = '
          f'{float(_gm75.float().mean())*100:.1f}% of the image  '
          f'(the leaky one would be ~99.7%)', flush=True)
    _, _m_un = render_mesh(_mfz, renderer, aligned=False)
    _un = (_m_un.squeeze(0) > 0.5)
    _iou_un = float((_un & _gm75).sum() / max(int((_un | _gm75).sum()), 1))
    _iou_al = float((render_mask & _gm75).sum()
                    / max(int((render_mask | _gm75).sum()), 1))
    print(f'\n[GATE-align] silhouette IoU vs GT at frame 75')
    print(f'  unaligned = {_iou_un:.4f}     aligned = {_iou_al:.4f}   '
          f'(+{_iou_al - _iou_un:.4f})', flush=True)
    assert _iou_al > _iou_un + 0.05, (
        f'GATE-align FAILED: alignment did not improve silhouette IoU '
        f'({_iou_un:.4f} -> {_iou_al:.4f}). The transform is not reaching the '
        f'renderer, or alignment.json is wrong.')
    print('[GATE-align] PASSED', flush=True)
    json.dump({'iou_unaligned': _iou_un, 'iou_aligned': _iou_al,
               'mask_px_stale': int(render_mask_stale.sum()),
               'mask_px_aligned': int(render_mask.sum()),
               'iou_stale_vs_aligned': _iou_masks,
               'scale': ALIGN_S, 'rotvec': ALIGN_ROTVEC, 't': ALIGN_T},
              open(_LOGD / 'alignment_applied.json', 'w'), indent=2)
    del _mfz, _slat75, _m_al, _m_un, _gm75, _a75, _img75
    gc.collect(); torch.cuda.empty_cache()

    # ── GATE-plain: B=0 → two-pass colonly render == frozen render ─────────────
    ...
    if resumed:
        ...
        # The parent called renderer.render() directly here. In rung13 that would
        # validate the UNALIGNED path while training runs the ALIGNED one — the
        # gate would still pass (alignment applies equally to both sides) and so
        # would silently certify a path nobody uses. Routed through render_mesh
        # so it tests exactly what the training loop does.
        slat_ref = _slat_from_cache(slat_cache, 75)
        with torch.no_grad():
            meshes_frz = dec_model(slat_ref)
        color_frz, _ = render_mesh(meshes_frz[0], renderer)
        color_frz    = color_frz.detach()

        mesh_co     = colonly_forward(dec_model, registry, slat_ref, with_grad=False)
        color_co, _ = render_mesh(mesh_co, renderer)
        color_co    = color_co.detach()

        diff = (color_frz - color_co).abs().max().item()
        print(f'  max |frozen - colonly(B=0)| = {diff:.3e}  (tol=1e-2)   '
              f'[both via render_mesh, i.e. both ALIGNED — the training path]')
        assert diff < 1e-2, f'GATE-plain FAILED: {diff:.3e}'
        print('[GATE-plain] PASSED\n')
        del meshes_frz, mesh_co, color_frz, color_co, slat_ref
        gc.collect(); torch.cuda.empty_cache()
    ...
    if not csv_path.exists():
        csv_path.write_text('epoch,loss_total,loss_mse,loss_lpips,held_psnr,'
                            'held_std,held_ssim,held_lpips,B_norm_mean,'
                            'B_norm_max,time_s\n')
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        with open(csv_path, 'a') as fh:
            fh.write(f'{epoch},{ep_loss:.6f},{ep_mse:.6f},{ep_lp:.6f},'
                     f'{held_psnr:.4f},{held_std:.4f},{held_ssim:.4f},'
                     f'{held_lp:.4f},{np.mean(bnorms):.5f},{np.max(bnorms):.5f},'
                     f'{ep_time:.1f}\n')
        save_curves(history)
