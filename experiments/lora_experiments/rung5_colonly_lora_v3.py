"""
rung5_colonly_lora_v3.py
------------------------
Color-only decoder LoRA — MLP-only (no attention LoRA).

Root cause of handle ghosting in v1/v2:
  Transformer self-attention is global across ALL voxels.
  Attention LoRA modifies QKV/out for body voxels → those modifications
  propagate to handle voxels through cross-voxel attention → handle
  features shift → out_layer outputs white for handle → ghost.
  L2 reg (v2) is too diluted (mean over 7301×48) to win against this.

Fix:
  Remove attention LoRA entirely. Only hook lora_fc1 and lora_fc2 (MLP layers).
  MLP layers are POSITION-WISE: each voxel is processed independently.
  Body voxels → body features in → lava colors learned → body bright.
  Handle voxels → handle features in → no loss signal → MLP LoRA leaves them alone.
  No cross-voxel contamination possible.

Config: all 12 decoder blocks, rank 4, seed 6, 30 epochs, MLP-only hooks.
"""
...
_LABEL = f'rung5_colonly_mlp_r{args.rank}_s{args.seed}_{RUN_ID}'
_OUT   = _RUNS / _LABEL
...
class DecBlockLoRABundle(nn.Module):
    """MLP-only LoRA — no qkv/out to avoid cross-voxel attention contamination."""
...
def dec_block_lora_ctx(dec_model, registry: DecLoRARegistry):
    """Hook only MLP layers — per-voxel, no cross-voxel contamination."""
    handles = []
...
def b_grad_norms(registry):
    ...
    for blk in registry.blocks.values():
        for lyr in (blk.lora_fc1, blk.lora_fc2):
            g = lyr.B.grad
            if g is not None:
...
def save_curves(history):
    ...
    fig.suptitle(f'Rung 5 colonly MLP-only — {_LABEL}', fontsize=11)
...
def main():
    ...
    print('=' * 72)
    print(f'Rung 5 Color-Only Decoder LoRA  v3 (MLP-only)')
    ...
    # MLP-only: 2 LoRA layers per block (fc1, fc2) instead of 4
    registry = DecLoRARegistry(ACTIVE_BLOCKS, rank=args.rank).to(DEVICE)
    n_params = sum(p.numel() for p in registry.parameters())
    print(f'\n[LORA] MLP-only  blocks={ACTIVE_BLOCKS}  rank={args.rank}  params={n_params:,}')

    # MLP-only: rank*(dim*4 + dim*4) per block = rank*6144 per block
    # fc1: dim→dim*4 = 768→3072, A=(rank,768), B=(3072,rank) = rank*(768+3072) = rank*3840
    # fc2: dim*4→dim = 3072→768, A=(rank,3072), B=(768,rank) = rank*(3072+768) = rank*3840
    # total per block = rank*7680, 12 blocks = rank*92160
    expected = args.rank * (DEC_DIM * 4 + DEC_DIM) * 2 * len(ACTIVE_BLOCKS)
    ...
    if not resumed:
        ...
        if n_zero:
            print(f'  WARNING: {n_zero}/{n_total} zero-grad B mats', flush=True)
        assert len(gnorms) == n_total, f'GATE 2 FAILED: {len(gnorms)}/{n_total}'
        optimizer.zero_grad()
        print('[GATE 2] PASSED\n', flush=True)
