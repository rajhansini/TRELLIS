...
rung22_kl_multi.py
------------------
rung21_kl.py made object-agnostic. rung21 is untouched and still runs the teapot.

THREE things were teapot-specific and are now arguments:
    --frames-dir   the GT video frames                (was hard-coded)
    --alignment    that object's alignment.json       (already an arg; now required)
    --ref-frame    frame the sparse structure is sampled from

AND ONE ASSERT WAS A LIE FOR ANY OTHER OBJECT.  rung21 asserts N_vox == 7301.
That is the teapot's voxel count from sample_sparse_structure on ITS reference
frame; every object has its own.  Here N_vox is recorded and sanity-bounded, not
equated to 7301.

Nothing else differs.  The KL, the gates, the two-stage backward, the
intersection loss region are rung21's.

rung21's header follows.
--------------------------------------------------------------------------------
rung21_kl.py
------------
rung20_kvonly.py + an ATTENTION-DISTRIBUTION KL to the frozen model.
rung20 is untouched.  --w-kl 0 reproduces rung20 exactly.

WHAT THE TERM IS
    KL( P_lora || P_frozen )  over the 1,374 DINO image tokens,
    averaged over the 1,748 voxel tokens and the 16 heads, per adapted block.

    loss = mse + W_LPIPS*lpips + w_kl * KL

WHY
    dL/dB currently sums over VISIBLE voxels only -- 2,278 of 7,301 (31.2%,
    measured).  The other 69% are changed by the shared B with no term in the
    gradient, so nothing measures whether the edit suits them.  Off-view
    brightness is 15.78% of pixels at yaw 270 against 2.01% at the trained
    view (yaw_bright_sweep.py, rung20 frame 1).

    Every voxel cross-attends whether or not a pixel scores it, so the KL is
    defined for all 7,301.  It constrains WHERE a voxel looks, not what it
    outputs: only K enters the softmax, V is left free, so the adapter can
    still change appearance on unseen surface.

PORTED FROM TRELLIS.2 experiments/dynamesh/kl_attn_lora.py, with three
differences that are NOT cosmetic:
    24 blocks, not 30
    1,374 DINO tokens, not 1,029
    qk_rms_norm is FALSE for cross-attention here.  blocks.py:136 passes
    qk_rms_norm=qk_rms_norm_cross, which defaults False and the
    slat_flow_img_dit_L_64l8p2 config never sets it.  The config's
    "qk_rms_norm": true is the SELF-attention flag.  Applying the norm would
    silently compute a different distribution than the model's own.

CALIBRATE BEFORE TRAINING.  TRELLIS.2 ran w_kl=1.0 and measured KL ~1.3e-4
against a loss of ~0.13, so the term contributed ~0.1% of the gradient and
those runs tested nothing.  --calibrate measures both terms with NO optimiser
step and prints the w_kl that buys a chosen fraction, then exits.

rung20's header follows.
--------------------------------------------------------------------------------
rung20_kvonly.py
----------------
...
_ap_.add_argument('--kl', action='store_true',
                  help='enable the attention-distribution KL to frozen.')
_ap_.add_argument('--w-kl', type=float, default=0.0,
                  help='beta. SET IT FROM --calibrate, never by guessing: '
                       'TRELLIS.2 used 1.0 and the term was 0.1% of the loss.')
_ap_.add_argument('--kl-blocks', default='all',
                  choices=['all', 'early', 'mid', 'late'],
                  help='which blocks contribute KL. Each retains a '
                       '[1748,16,1374] fp32 logit graph for the backward '
                       '(~154 MB); subsets are the fallback if that does not fit.')
_ap_.add_argument('--kl-voxels', type=int, default=0,
                  help='0 = all 1,748 voxel tokens. >0 subsamples that many per '
                       'call -- an unbiased estimate of the same mean, at lower '
                       'memory. Sampling is per call, so every voxel is seen.')
_ap_.add_argument('--calibrate', action='store_true',
                  help='measure KL and loss magnitudes, print the implied w_kl, '
                       'and EXIT. Takes no optimiser step and writes no ckpt.')
_ap_.add_argument('--calib-steps', type=int, default=40)
_ap_.add_argument('--frames-dir', default=None,
                  help="this object's GT frames. Default = the teapot's.")
_ap_.add_argument('--ref-frame', type=int, default=75,
                  help='frame the sparse structure is sampled from. Must match '
                       'the one build_slat_cache_obj.py used, or the alignment '
                       'was solved against a different mesh.')
_ap_.add_argument('--obj', default='teapot',
                  help='short tag; goes in the run label so objects do not '
                       'collide in runs/.')
_ap_.add_argument('--init-from', default=None,
                  help='load a trained registry before calibrating. REQUIRED for '
                       '--calibrate: a fresh registry is B=0, so the KL is '
                       'identically zero and there is no ratio to measure. Point '
                       'it at rung20 lora_best.pt to calibrate at the operating '
                       'point rung20 actually reached.')
...
GT_FRAMES_DIR = Path(args.frames_dir) if args.frames_dir else Path(
...
_CFG = dict(
    ...
    obj=args.obj, frames_dir=str(GT_FRAMES_DIR), ref_frame=args.ref_frame,
    kl=bool(args.kl), w_kl=float(args.w_kl), kl_blocks=args.kl_blocks,
    kl_voxels=int(args.kl_voxels),
...
_LABEL = (f'rung22_{args.obj}_{args.loss_region}_{args.tsample}_{args.blocks}'
...
# ── the attention-distribution KL ─────────────────────────────────────────────

# Collected ONLY inside the graded stage-2 flow evaluation. The no-grad prefix,
# every gate, and evaluate_frames all run with collect False, so nothing outside
# the training gradient ever appends a term or holds a graph.
_KL = {'terms': [], 'collect': False, 'blocks': set()}
_KLB = {'all':   list(range(N_XATTN)),
        'early': list(range(0, N_XATTN // 3)),
        'mid':   list(range(N_XATTN // 3, 2 * N_XATTN // 3)),
        'late':  list(range(2 * N_XATTN // 3, N_XATTN))}[args.kl_blocks]
_KL['blocks'] = set(_KLB)


def attn_kl(module, q_feats, kv_frozen, kv_adapted):
    """
    KL( P_lora || P_frozen ) over the image tokens, averaged over voxels+heads.

    SHAPES, from the model's own arithmetic (modules.py:126-139):
        q_feats    [N, C]        N = 1748 voxel tokens, C = 1024. No batch dim:
                                 _linear calls module(x.feats), so this is the
                                 plain tensor, not a SparseTensor.
        kv_*       [1, Lkv, 2C]  Lkv = 1374 DINO tokens, context is dense.
        -> reshape (..., 2, H, D), unbind(-3)[0] takes K.  H=16, D=64.

    qk_rms_norm is FALSE for cross-attention on this checkpoint (blocks.py:136
    passes qk_rms_norm_cross, default False, never set by the config). The guard
    below is kept so a checkpoint that DOES enable it uses the norm rather than
    silently computing a different distribution.

    ON THE QUERIES.  q comes from the ADAPTED forward and is shared by both
    arms.  With --targets kv, to_q carries no adapter, so at block 0 this is
    exactly the frozen query; deeper blocks see an input already changed
    upstream and the frozen model's true q would differ.  Recovering it needs a
    second full forward.  Holding q fixed measures the divergence ATTRIBUTABLE
    TO THE ADAPTED KEYS -- the only thing this adapter controls.  Deliberate.

    The frozen arm is under no_grad: it is a reference, not something to move.
    """
    H = module.num_heads
    D = q_feats.shape[-1] // H
    q = q_feats.reshape(-1, H, D)
    if args.kl_voxels > 0 and q.shape[0] > args.kl_voxels:
        sel = torch.randint(0, q.shape[0], (args.kl_voxels,), device=q.device)
        q = q.index_select(0, sel)
    if module.qk_rms_norm:
        q = module.q_rms_norm(q)

    def logits(kv):
        k = kv.reshape(*kv.shape[:-1], 2, H, D).unbind(dim=-3)[0]   # [1,Lkv,H,D]
        if module.qk_rms_norm:
            k = module.k_rms_norm(k)
        k = k.reshape(-1, H, D)
        return torch.einsum('nhd,khd->nhk', q.float(), k.float()) * (1.0 / math.sqrt(D))

    with torch.no_grad():
        lp_f = torch.log_softmax(logits(kv_frozen), dim=-1)
    lp_a = torch.log_softmax(logits(kv_adapted), dim=-1)
    return (lp_a.exp() * (lp_a - lp_f)).sum(-1).mean()


# ── the patched cross-attention forward ───────────────────────────────────────

def _xattn_fwd(module, x, context, lb, idx=None):
    """
    ...
    _kv_frozen = kv_t                      # pre-LoRA keys: the KL's reference
    if lb is not None:
        l_kv = lb.get('to_kv')
        if l_kv is not None:
            kv_t = kv_t + l_kv(context).to(kv_t.dtype)
    # THE RUNG-21 ADDITION. `kv_t is not _kv_frozen` means a delta was actually
    # added; without it an unadapted block would contribute an exact zero and
    # drag the mean down.
    if _KL['collect'] and idx in _KL['blocks'] and kv_t is not _kv_frozen:
        _KL['terms'].append(attn_kl(module, q_sp.feats, _kv_frozen, kv_t))
    ...
    for blk in flow_model.blocks:
        ...
        def _make(mod, lb_, idx_):
            def _fwd(x, context=None):
                return _xattn_fwd(mod, x, context, lb_, idx_)
            return _fwd

        ca.forward = _make(ca, lb, ca_idx)
        ca_idx += 1
...
def main():
    ...
    ref_img     = Image.open(
        GT_FRAMES_DIR / f'frame_{args.ref_frame:04d}.png').convert('RGB')
    ...
    # 7301 is the TEAPOT's count. Every object gets its own from its own
    # reference frame, so this bounds it instead of equating it.
    assert 500 < N_vox < 60000, (
        f'N_vox={N_vox} is implausible; the structure sampler failed or '
        f'--ref-frame {args.ref_frame} is not a real frame of this object.')
    print(f'  (teapot is 7,301; this object has its own)', flush=True)
    ...
    # AFTER every B=0 gate. GATE-identity asserts the patched flow reproduces
    # frozen when B=0; loading trained weights before it makes that gate compare
    # frozen against a trained adapter and fail for the wrong reason.
    if args.init_from:
        _ck0 = torch.load(args.init_from, map_location='cpu', weights_only=True)
        registry.load_state_dict(_ck0['registry_state'], strict=True)
        _bm = max(float(getattr(b, f'lora_{n}').B.float().norm())
                  for b in registry.blocks.values() for n in b.targets)
        assert _bm > 1e-6, f'--init-from gave an all-zero B ({_bm:.3e})'
        print(f'[INIT] {args.init_from}  epoch {_ck0.get("epoch")}  '
              f'psnr {_ck0.get("best_psnr", float("nan")):.3f}  max||B|| {_bm:.4f}',
              flush=True)
        del _ck0
    ...
    if args.kl:
        ...
        print('\n[GATE-kl] attention-distribution KL', flush=True)
        print(f'  blocks contributing : {sorted(_KL["blocks"])}', flush=True)
        print(f'  voxels per call     : '
              f'{"all 1,748" if args.kl_voxels == 0 else args.kl_voxels}', flush=True)

        # (a) B=0 must give EXACTLY zero KL: kv_adapted == kv_frozen bit for bit.
        # If this is nonzero the reference arm is wrong and every number after
        # it is meaningless.
        _saved = {k: v.detach().clone() for k, v in registry.state_dict().items()}
        with torch.no_grad():
            for _n, _prm in registry.named_parameters():
                if _n.endswith('.B'):
                    _prm.zero_()
        _KL['terms'].clear(); _KL['collect'] = True
        with torch.no_grad():
            _xk0 = sp.SparseTensor(feats=fixed_noise_feats.clone(), coords=coords)
            flow_eval_at(flow_model, _xk0, T_PAIRS[0][0],
                         raw_tokens[DIAG_FRAMES[0]].unsqueeze(0).to(DEVICE),
                         registry, False)
        _KL['collect'] = False
        _z = [float(t) for t in _KL['terms']]; _KL['terms'].clear()
        assert _z, 'GATE-kl FAILED: B=0 probe collected no KL terms at all.'
        assert max(abs(v) for v in _z) < 1e-9, (
            f'GATE-kl FAILED: with B=0 the KL should be exactly 0, got '
            f'max {max(abs(v) for v in _z):.3e} over {len(_z)} blocks. The '
            f'frozen reference is not the frozen keys.')
        print(f'  B=0 -> KL max {max(abs(v) for v in _z):.3e} over '
              f'{len(_z)} blocks   PASSED', flush=True)
        registry.load_state_dict(_saved)
        del _saved, _z
        gc.collect(); torch.cuda.empty_cache()

        # (b) measure both terms with NO optimiser step, so w_kl is set from the
        # measured ratio instead of guessed. TRELLIS.2 guessed 1.0 and the term
        # was 0.1% of the loss.
        _measurable = bool(args.calibrate or args.init_from)
        if not _measurable:
            print('  B is zero-init, so KL starts at exactly 0 and grows with '
                  'training; the ratio is only measurable at a trained point.\n'
                  f'  ratio was calibrated separately: see kl_calibration.json.\n'
                  f'  training with --w-kl {args.w_kl:.4g}', flush=True)
            assert args.w_kl > 0, (
                'GATE-kl FAILED: --kl is on but --w-kl is 0, so the term would '
                'be logged and never applied. Pass a weight or drop --kl.')
            print('[GATE-kl] PASSED', flush=True)
        else:
            assert not args.calibrate or args.init_from, (
                'GATE-kl FAILED: --calibrate on a fresh registry measures nothing. '
                'B is zero-init, so KL is identically 0. Pass --init-from '
                '<rung20 lora_best.pt> to calibrate at a real operating point.')
            _wkl_saved = args.w_kl
            args.w_kl = 0.0                      # measure only; no KL gradient
            _kls, _tots = [], []
            _n_cal = args.calib_steps if args.calibrate else 8
            print(f'  measuring over {_n_cal} steps (no optimiser step)...', flush=True)
            for _i in range(_n_cal):
                _r = _train_step(flow_model, dec_model, registry, renderer, lpips_fn,
                                 fixed_noise_feats, coords, raw_tokens,
                                 TRAIN[_i % len(TRAIN)], render_mask,
                                 loss_scale, trainable, optimizer, False)
                if _r is None or not _r.get('kl_n'):
                    continue
                _kls.append(_r['kl']); _tots.append(_r['total'])
            optimizer.zero_grad(set_to_none=True)
            args.w_kl = _wkl_saved
            assert _kls, 'GATE-kl FAILED: no step produced a KL term.'
            _klm  = sum(_kls) / len(_kls)
            _totm = sum(_tots) / len(_tots)
            assert _klm > 0, f'GATE-kl FAILED: mean KL is {_klm:.3e}, adapter inert?'
            print(f'  mean KL   {_klm:.6e}   over {len(_kls)} steps', flush=True)
            print(f'  mean loss {_totm:.6e}   (mse + {W_LPIPS}*lpips)', flush=True)
            print(f'  ratio loss/KL = {_totm / _klm:.4g}', flush=True)
            print('  w_kl for a target KL share of the total:', flush=True)
            for _f in (0.01, 0.05, 0.10, 0.25, 0.50):
                print(f'     {_f*100:5.1f}%  ->  w_kl = {_f / (1 - _f) * _totm / _klm:.4g}',
                      flush=True)
            print(f'  current --w-kl {args.w_kl:.4g}  ->  KL is '
                  f'{100 * args.w_kl * _klm / (_totm + args.w_kl * _klm):.3f}% of the total',
                  flush=True)
            print(f'  peak GPU {torch.cuda.max_memory_allocated()/2**30:.2f} GiB', flush=True)
            json.dump({'mean_kl': _klm, 'mean_loss': _totm, 'ratio': _totm / _klm,
                       'steps': len(_kls), 'kl_blocks': args.kl_blocks,
                       'kl_voxels': args.kl_voxels, 'w_kl': args.w_kl,
                       'suggested': {f'{int(f*100)}pct': f / (1 - f) * _totm / _klm
                                     for f in (0.01, 0.05, 0.10, 0.25, 0.50)}},
                      open(_LOGD / 'kl_calibration.json', 'w'), indent=2)
            print('[GATE-kl] PASSED', flush=True)
            if args.calibrate:
                print('\n[CALIBRATE] measurement only — exiting before training.',
                      flush=True)
                return
            assert args.w_kl > 0, (
                'GATE-kl FAILED: --kl is on but --w-kl is 0, so the term would be '
                'logged and never applied. Pass a weight or drop --kl.')
...
def _train_step(flow_model, dec_model, registry, renderer, lpips_fn,
    ...
    # ONE backward with two roots. grad_slat already carries loss_scale (stage 1
    # scaled by it), so the KL's grad_tensor must carry it too or the two terms
    # are weighted differently; the later p.grad.div_(loss_scale) undoes both.
    _klv, _kln = 0.0, len(_kl_terms)
    if args.kl and _kl_terms:
        _kl_mean = torch.stack(_kl_terms).mean()
        _klv = float(_kl_mean.detach())
        assert math.isfinite(_klv), f'frame {fi} k{k}: KL is not finite ({_klv})'
        torch.autograd.backward(
            [slat_r.feats, _kl_mean],
            [grad_slat, torch.tensor(args.w_kl * loss_scale,
                                     device=DEVICE, dtype=_kl_mean.dtype)])
        del _kl_mean
    else:
