"""
rung5_colonly_lora.py
---------------------
Decoder LoRA trained with color-only supervision.

Problem with rung5_subladder (5.4):
  LoRA learns joint geometry+color changes. Post-hoc splice (frozen geom + LoRA colors)
  puts the mesh extractor into an OOD state → handle artifacts.

Fix:
  Train and infer using the SAME two-pass out_layer interception:
    1. frozen pass (no_grad)  → capture out_layer feats → frozen geometry (ch 0:53)
    2. LoRA pass   (with grad) → capture out_layer feats → LoRA colors   (ch 53:101)
    3. mixed = cat([frozen_geom.detach(), lora_col.clamp(ABS_MIN, ABS_MAX)], dim=1)
    4. to_representation(mixed) → mesh → render → loss → backward

  Gradients ONLY flow through color channels → LoRA learns colors, not geometry.
  At inference, same two-pass → no OOD combination → clean renders.

Abs clamp (not delta):
  sigmoid(8.0) = 0.9997  → bright orange, not white
  sigmoid(-9.0) = 0.0001 → near-black, no artificial shadow

Config: all 12 decoder blocks, rank 4, seed 6, 30 epochs.
Resume: SLaT cache saved to disk after first run; epoch checkpoints for node-loss recovery.

Usage:
  python rung5_colonly_lora.py --rank 4 --epochs 30 --seed 6 \
    2>&1 | tee slurm_logs/colonly_$(date +%Y%m%d_%H%M%S).log
"""
...
_LABEL = f'rung5_colonly_r{args.rank}_s{args.seed}_{RUN_ID}'
_OUT   = _RUNS / _LABEL
