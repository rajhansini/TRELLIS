...
_LABEL = (f'rung14v1_{args.blocks}_{args.targets}_r{args.rank}_s{args.seed}_{RUN_ID}')
_OUT   = _RUNS / _LABEL
...
def denoise_prefix_nograd(flow_model, noise_sp, cond_gl, registry):
    """Steps 1..24 with the LoRA ACTIVE but no gradient retained."""
    x = noise_sp
    with torch.no_grad():
...
def denoise_last_step(flow_model, x_in, cond_gl, registry, require_grad: bool):
    t, t_prev = T_PAIRS[-1]
    t_ten = torch.tensor([1000.0 * t], device=DEVICE, dtype=torch.float32)
...
def main():
    ...
    print('=' * 72)
    print('rung14-v1 — LoRA on SLaT-flow CROSS-ATTENTION (before the decoder)')
    ...
    if not resumed:
        print('\n[GATE-grad] gradient reachability...', flush=True)
        optimizer.zero_grad(set_to_none=True)
        torch.cuda.reset_peak_memory_stats()
        probe = _train_step(flow_model, dec_model, registry, renderer, lpips_fn,
                            fixed_noise_feats, coords, raw_tokens, 75,
                            render_mask, loss_scale, trainable,
                            optimizer, do_step=False)
        assert probe is not None, (
            'GATE-grad FAILED: the probe step returned nothing — either no '
            'gradient reached the SLaT leaf, or the grads were non-finite. '
            'Both are fatal; see the [f075] line above for which.')
        st = probe['gstats']
        # Printed first because an OOM 4 hours in is the expensive way to learn
        # that 24 adapted blocks do not fit.
        print(f'  peak GPU during one real training step: '
              f'{torch.cuda.max_memory_allocated()/1e9:.2f} GB', flush=True)
        print(f'  per-target ||dB||: ' +
              '  '.join(f'{k}={v:.4e}' for k, v in st['per_target'].items()))
        print(f'  min={st["min"]:.3e}  med={st["med"]:.3e}  max={st["max"]:.3e}  '
              f'spread={st["ratio"]:.1f}x')
        print(f'  zero-gradient matrices: {st["n_zero"]}/{st["n_total"]}')
        dead = [b for b, v in sorted(st['per_block'].items()) if v == 0.0]
        assert st['n_zero'] == 0, (
            f'GATE-grad FAILED: {st["n_zero"]}/{st["n_total"]} B matrices get '
            f'zero gradient. Dead blocks: {dead}. Training would be a no-op for '
            f'them. Raise LOSS_SCALE or reduce the block set.')
        print('[GATE-grad] PASSED\n', flush=True)
        optimizer.zero_grad(set_to_none=True)
        st['peak_gpu_gb'] = torch.cuda.max_memory_allocated() / 1e9
        json.dump(st, open(_LOGD / 'gate_grad.json', 'w'), indent=2)
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        for fi in order:
            ...
            if n_steps % 15 == 0:
                print(f'  e{epoch:02d} [{n_steps:03d}/{len(TRAIN)}] f{fi:03d}  '
                      f'mse={r["mse"]:.5f}  lpips={r["lp"]:.5f}  '
                      f'total={r["total"]:.5f}  nz={r["gstats"]["n_zero"]}',
                      flush=True)
        ...
        print(f'\n[EPOCH {epoch}/{EPOCHS}]  loss={ep_loss:.5f}  '
              f'(mse={ep_mse:.5f}  lpips x0.1={ep_lp*W_LPIPS:.5f})', flush=True)
...
def _train_step(flow_model, dec_model, registry, renderer, lpips_fn,
                ...
                loss_scale, trainable, optimizer, do_step: bool):
    """
    One frame. Two-stage backward so the flow graph and the decoder graph never
    co-exist on the GPU (rung2's strategy; flow_model is ~4 GB).
    """
    cond = raw_tokens[fi].unsqueeze(0).to(DEVICE)
    optimizer.zero_grad(set_to_none=True)

    noise_sp = sp.SparseTensor(feats=noise_feats.clone(), coords=coords)
    x_prefix = denoise_prefix_nograd(flow_model, noise_sp, cond, registry)

    with torch.no_grad():
        x0_ng   = denoise_last_step(flow_model, x_prefix, cond, registry, False)
    ...
    if leaf.grad is None:
        print(f'  f{fi:03d} SKIP: no grad reached the SLaT leaf', flush=True)
        optimizer.zero_grad(set_to_none=True)
        gc.collect(); torch.cuda.empty_cache()
        return None
    ...
    # stage 2 — replay step 25 with grad, inject the SLaT gradient
    x0_raw = denoise_last_step(flow_model, x_prefix, cond, registry, True)
    ...
    bad = any(p.grad is not None and not torch.isfinite(p.grad).all()
              for p in trainable)
    if bad:
        print(f'  f{fi:03d} non-finite grad — step skipped', flush=True)
        optimizer.zero_grad(set_to_none=True)
        gc.collect(); torch.cuda.empty_cache()
        return None
    ...
    del x0_raw, slat_r, grad_slat, x_prefix, x0_ng, slat_ng, noise_sp, cond
    gc.collect(); torch.cuda.empty_cache()
    return {**vals, 'gstats': gstats}
