#!/bin/bash
# Submit rung5 color-only decoder LoRA training.
# Usage: bash submit_rung5_colonly.sh [--rank R] [--epochs E] [--seed S]

set -e

PY=/net/projects/ranalab/rajhansini/conda_envs/trellis/bin/python
SCRIPT=/net/projects/ranalab/rajhansini/TRELLIS/experiments/lora_experiments/rung5_colonly_lora.py
...
echo "=== Rung 5 Color-Only Decoder LoRA ==="
...
TRAIN_JOB=$(sbatch \
    ...
    --output="${LOGDIR}/rung5_colonly_%j.log" \
    --error="${LOGDIR}/rung5_colonly_%j.log" \
    --wrap="
set -e
...
${PY} -u ${SCRIPT} --rank ${RANK} --epochs ${EPOCHS} --seed ${SEED}
echo '=== Done ==='
" | awk '{print $NF}')
...
echo "  tail -f ${LOGDIR}/rung5_colonly_${TRAIN_JOB}.log"
