...
def main():
    ...
    if _slat_cache_disk.exists():
        # Resume path: load pre-computed SLaTs, skip expensive flow model forward passes.
        ...
        flow_model.cpu()
        # Save to disk so future resumes skip flow model computation entirely.
        _sarr = np.stack([slat_cache[fi][0].numpy() for fi in range(1, N_FRAMES + 1)])
        np.savez_compressed(_slat_cache_disk,
                            slats=_sarr, coords=coords.cpu().numpy())
