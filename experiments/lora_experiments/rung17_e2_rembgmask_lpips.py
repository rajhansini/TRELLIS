"""
rung17_e2_rembgmask_lpips.py
----------------------------
rung16_e1_rembgmask.py with ONE further change: W_LPIPS 0.1 -> 0.25.
rung16 and rung15 are untouched and still run.

WHY BOTH CHANGES, AND WHY IN THIS ORDER
  Fixing the mask alone (rung16) multiplies MSE by ~8.7x and moves the balance
  from 1.8:1 to ~15:1 -- MSE strongly dominant.  That guards brightness, which is
  what rung15 lost, but it also gives LPIPS less relative weight than rung15 had,
  so the perceptual pressure that makes lava look like lava drops.

  Raising W_LPIPS to 0.25 on top puts the balance at ~6:1: MSE still clearly in
  charge of absolute values, LPIPS carrying 2.5x the weight rung16 gives it.

      run     mask     W       MSE term   W*LPIPS    MSE:LPIPS
      rung15  leaky    0.10    0.00333    0.00187      1.8 : 1
      rung16  rembg    0.10    0.0288     0.00187     15.4 : 1
      rung17  rembg    0.25    0.0288     0.00468      6.2 : 1

  W = 1.0 would return to ~1.5:1, i.e. essentially rung15's balance and its
  failure, so the useful range is bounded above.

  This run is TWO changes from rung15 and ONE from rung16.  It is only
  interpretable alongside rung16; on its own it cannot say which change did what.
...
random-knot gradient, rung13 alignment, same held-out split.
...
LOSS_SCALE0  = 4096.0
GRAD_CLIP    = 1.0
W_LPIPS      = 0.25      # rung16 used 0.1; THE RUNG-17 CHANGE
...
_CFG = dict(
    variant='v1_crossattn_lora_randt_rembgmask_lpips025',
...
_LABEL = (f'rung17e2_{args.train_mask}_{args.tsample}_{args.blocks}'
...
def main():
    ...
    print('=' * 72)
    print('rung17-E2 — rung16 (mask fixed) + W_LPIPS 0.1 -> 0.25')
