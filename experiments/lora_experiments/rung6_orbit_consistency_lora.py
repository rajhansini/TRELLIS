"""
rung6_orbit_consistency_lora.py
--------------------------------
v4 out_layer color LoRA + orbit consistency loss.

Problem with v4/v5:
  Single front-view supervision → LoRA pastes texture like a sticker.
  Texture looks correct from front but is not 3D-coherent around the object.

Fix (rung6):
  After decoding the LoRA mesh (ONE forward pass through decoder), render it
  from K random orbit angles (no extra decoder forward pass needed — just
  re-render the same mesh from different cameras).
  Orbit loss = MSE( LoRA_render@theta , frozen_render@theta ).
  This penalizes the LoRA for changing anything visible from non-front views.
  Result: LoRA matches the video from the front, preserves frozen appearance
  everywhere else → 3D-coherent texture, lower sticker effect.

  lambda_orbit controls the trade-off:
    high  → very 3D-coherent, lower front-view fidelity
    low   → more front-view fidelity, some sticker effect remains
"""
...
_ap_.add_argument('--lambda-orbit',  type=float, default=0.5,
                  help='Weight for orbit consistency loss (0=disabled)')
_ap_.add_argument('--orbit-k',       type=int,   default=2,
                  help='Number of random orbit angles sampled per training step')
...
W_LPIPS       = 0.1
DEC_DIM       = 768
DEC_OUT_IN_DIM = DEC_DIM // 8   # 96 — out_layer input dim after 2× upsample
COLOR_START   = 53
COLOR_END     = 101
...
# Orbit consistency — angles (degrees) sampled during training.
# 0° = front view (already supervised by GT video).
# We penalize any deviation from frozen decoder at these non-front angles.
LAMBDA_ORBIT = args.lambda_orbit
ORBIT_K      = args.orbit_k
ORBIT_ANGLES = [45, 90, 135, 180, 225, 270, 315]  # 7 non-front angles

# ── run identity ───────────────────────────────────────────────────────────────
_CFG = dict(
    variant='color_only_outlayer_orbit',
    ...
    lambda_orbit=LAMBDA_ORBIT, orbit_k=ORBIT_K,
    held_out=HELD_OUT,
)
RUN_ID = hashlib.md5(json.dumps(_CFG, sort_keys=True).encode()).hexdigest()[:8]
_LABEL = f'rung6_orbit_r{args.rank}_s{args.seed}_lo{LAMBDA_ORBIT}_{RUN_ID}'
_OUT   = _RUNS / _LABEL
...
# ── LoRA module — out_layer only ───────────────────────────────────────────────

class OutLayerLoRA(nn.Module):
    """
    LoRA adapter hooked on dec_model.out_layer (SparseLinear, 96→101).
    Maps each voxel's 96-dim pre-out_layer feature to a 48-dim color delta.
    Operates independently per voxel — no cross-voxel contamination possible.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (N_vox, in_dim)  →  delta: (N_vox, color_dim)
        return ((x.float() @ self.A.T) @ self.B.T).to(x.dtype)


# ── Color-only single-pass forward ────────────────────────────────────────────
...
def colonly_forward(dec_model, lora: OutLayerLoRA, slat_norm,
                    with_grad=True, return_feats=False):
    """
    Single forward pass through the frozen decoder.
    out_layer hook intercepts the output:
      - captures frozen_col (before delta) for L2 reg
      - adds LoRA delta to color channels [53:101]
      - clamps total to [ABS_MIN, ABS_MAX]
      - geometry channels [0:53] are untouched
    """
    captured = {}

    def _hook(mod, inp, out):
        # inp[0]: SparseTensor, inp[0].feats: (N_fine_vox, DEC_OUT_IN_DIM=96)
        # out:    SparseTensor, out.feats:    (N_fine_vox, 101)
        x_feats = inp[0].feats                            # (N, 96)
        delta   = lora(x_feats)                           # (N, 48)

        new_feats  = out.feats.clone()
        frozen_col = new_feats[:, COLOR_START:COLOR_END]  # (N, 48) — for reg/capture
        raw_col    = frozen_col + delta                   # (N, 48) — unclamped

        captured['frozen_col'] = frozen_col.detach()
        captured['raw_col']    = raw_col                  # keeps grad

        new_feats[:, COLOR_START:COLOR_END] = raw_col.clamp(ABS_MIN, ABS_MAX)
        # geometry channels [0:53] are unchanged
...
def make_orbit_extrinsics(theta_deg):
    """
    Extrinsics for a camera orbiting around the object's vertical (Z) axis.
    theta=0   → front view  (matches EXTRINSICS exactly)
    theta=90  → right side
    theta=180 → back
    theta=270 → left side

    Derivation: camera sits on the circle of radius 2 in the XY plane.
    Front view has cam at world-Y=-2 looking toward +Y (object at origin).
    Rotating by theta around world-Z:
      R(theta) = [[ cos,  sin,  0 ],
                  [  0,    0,  -1 ],
                  [-sin,  cos,  0 ]]
      t = [0, 0, 2]  (constant — camera always 2 units from origin)
    """
...
def render_mesh_ext(mesh, renderer, ext):
    """Render mesh with arbitrary extrinsics (no degenerate-face filter — mesh already clean)."""
    res  = renderer.render(mesh, ext.to(DEVICE), INTRINSICS.to(DEVICE),
                           return_types=['color', 'mask'])
    mask = res['mask'].unsqueeze(0)
    return (res['color'] * mask + (1.0 - mask)).detach()


def precompute_frozen_orbit_renders(dec_model, slat_cache, renderer, frame_list):
    """
    For every training frame, render the FROZEN decoder from each ORBIT_ANGLE.
    These become the targets for the orbit consistency loss.
    Decode once per frame (expensive) → render K angles (cheap, same mesh).
    """
    if LAMBDA_ORBIT <= 0 or not ORBIT_ANGLES:
        return {}
    print(f'\n[ORBIT] Precomputing frozen renders at {ORBIT_ANGLES}° for {len(frame_list)} frames...',
          flush=True)
    orbit_exts = {th: make_orbit_extrinsics(th) for th in ORBIT_ANGLES}
    frozen_orbit = {}
    dec_model.eval()
    for k, fi in enumerate(frame_list):
        ...
        with torch.no_grad():
            ...
            frozen_orbit[fi] = {
                th: render_mesh_ext(mesh, renderer, ext)
                for th, ext in orbit_exts.items()
            }
        del mesh, meshes, slat_norm
        if (k + 1) % 50 == 0 or k == len(frame_list) - 1:
            print(f'  {k+1}/{len(frame_list)}', flush=True)
        gc.collect(); torch.cuda.empty_cache()
    print('[ORBIT] Precompute done.', flush=True)
    return frozen_orbit
...
def save_diagnostics(dec_model, lora, slat_cache, renderer, epoch, baseline_cache):
...
def save_curves(history):
    ...
    fig.suptitle(f'Rung 5 colonly out_layer hook — {_LABEL}', fontsize=11)
...
def main():
    ...
    print('=' * 72)
    print(f'Rung 6 Orbit Consistency LoRA (out_layer hook + orbit loss)')
    print(f'  LoRA position : dec_model.out_layer  (after upsamplers)')
    print(f'  in_dim        : {DEC_OUT_IN_DIM}  (model_channels//8)')
    ...
    print(f'  lambda_orbit  : {LAMBDA_ORBIT}  (orbit consistency weight)')
    print(f'  orbit_k       : {ORBIT_K}  (random orbit angles per step)')
    print(f'  orbit_angles  : {ORBIT_ANGLES}')
    ...
    print(f'[ARCH] expected DEC_OUT_IN_DIM={DEC_OUT_IN_DIM}')
    ...
    # single OutLayerLoRA — rank*(96+48) = rank*144 params
    ...
    print(f'[GATE 0] param count PASSED ({n_params:,} == rank*{DEC_OUT_IN_DIM + COLOR_DIM})\n')
    ...
    # reuse slat cache from v4/v5 run (same seed/struct → identical SLATs)
    _candidate_caches = [
        _HERE / 'runs' / 'rung5_colonly_outlayer_r4_s6_c85c888f' / 'slat_cache.npz',  # v4
        _HERE / 'runs' / 'rung5_colonly_r4_s6_5d6b1700'          / 'slat_cache.npz',  # v1
        _HERE / 'runs' / 'rung5_colonly_mlp_r4_s6_0931a3d8'      / 'slat_cache.npz',  # v3
    ]
    ...
    if _slat_cache_disk.exists():
        ...
        _found_cache = next((c for c in _candidate_caches if c.exists()), None)
        if _found_cache:
            print(f'\n[SLAT CACHE] Reusing cache: {_found_cache}', flush=True)
            _npz      = np.load(_found_cache)
            _sarr     = _npz['slats']
            _coords_t = torch.from_numpy(_npz['coords']).int()
            slat_cache = {fi: (torch.from_numpy(_sarr[fi - 1]).float(), _coords_t)
                          for fi in range(1, N_FRAMES + 1)}
            ...
            print(f'\n[SLAT CACHE] Saved → {_slat_cache_disk}', flush=True)
    ...
    if not resumed:
        print('\n[GATE-plain] B=0 identity check (out_layer hook, B=zeros → delta=0)...')
    ...
    if not resumed:
        ...
        mesh, raw_lc, frz_c = colonly_forward(
            dec_model, lora, slat_75, with_grad=True, return_feats=True)
        color, _ = render_mesh(mesh, renderer)
    ...
    dec_model.eval()
    baseline_cache   = {}
    frozen_orbit_cache = precompute_frozen_orbit_renders(dec_model, slat_cache, renderer, TRAIN)

    print(f'[TRAIN] epochs {start_epoch}→{EPOCHS}  frames/epoch={len(TRAIN)}'
          f'  LOSS_SCALE={loss_scale:.0f}  lambda_reg={LAMBDA_REG}'
          f'  lambda_orbit={LAMBDA_ORBIT}', flush=True)
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        for fi in frame_order:
            ...
            # ONE decoder forward pass — gives LoRA mesh for ALL render angles
            mesh, raw_lora_col, frozen_col = colonly_forward(
                dec_model, lora, slat_norm, with_grad=True, return_feats=True)

            # ── Front-view loss (supervised by GT video frame) ──────────────
            color, _ = render_mesh(mesh, renderer)
            gt, gt_mask = load_gt(fi)
            mse, lp, render_loss = masked_loss(color, gt, render_mask, gt_mask, lpips_fn)

            # ── Color-delta regularisation ───────────────────────────────────
            reg = LAMBDA_REG * (raw_lora_col - frozen_col).pow(2).mean()

            # ── Orbit consistency loss ───────────────────────────────────────
            # Re-render the SAME LoRA mesh from K random orbit angles.
            # Target = frozen decoder render at that angle (precomputed).
            # No extra decoder forward pass needed.
            orbit_loss = torch.tensor(0.0, device=DEVICE)
            if LAMBDA_ORBIT > 0 and frozen_orbit_cache and fi in frozen_orbit_cache:
                thetas = random.sample(ORBIT_ANGLES, min(ORBIT_K, len(ORBIT_ANGLES)))
                mesh_clean = filter_degenerate_faces(mesh)
                for th in thetas:
                    ext           = make_orbit_extrinsics(th).to(DEVICE)
                    lora_view     = render_mesh_ext(mesh_clean, renderer, ext)
                    frozen_view   = frozen_orbit_cache[fi][th]
                    orbit_loss    = orbit_loss + F.mse_loss(lora_view, frozen_view)
                orbit_loss = orbit_loss / len(thetas)

            total = render_loss + reg + LAMBDA_ORBIT * orbit_loss
            ...
            if n_steps % 15 == 0:
                print(f'  e{epoch:02d} [{n_steps:03d}/{len(TRAIN)}] f{fi:03d}  '
                      ...
                      f'reg={reg.item():.5f}  orbit={orbit_loss.item():.5f}  '
                      f'total={total.item():.5f}', flush=True)

            del mesh, color, gt, gt_mask, mse, lp, render_loss, reg, orbit_loss, total
