"""
rung5_colonly_lora_v4.py
------------------------
Color-only decoder LoRA — out_layer hook only.

Root cause of handle ghosting in v1/v2/v3:
  v1/v2: LoRA inside SparseTransformerBlocks (QKV/out or MLP).
         Transformer attention mixes features across ALL voxels → body
         modifications propagate to handle through attention → contamination.
  v3:    MLP-only LoRA (no attention). Contamination still happens through
         SparseSubdivideBlock3d upsamplers: LoRA modifies 7301 coarse voxels →
         frozen upsamplers spread corrupted features to handle region in ~58k
         fine voxels → out_layer maps handle to white.

Fix (v4):
  Hook LoRA DIRECTLY ON out_layer — AFTER both transformer blocks AND upsamplers.
  out_layer maps (N_fine, 96) → (N_fine, 101).
  LoRA computes delta from each voxel's own 96-dim feature independently.
  Delta is added ONLY to color channels [53:101].
  Each fine voxel's delta depends only on its own features — zero cross-voxel
  contamination, zero upsampler contamination.
  Single forward pass (no two-pass needed: geometry [0:53] is untouched by hook).
"""
...
_LABEL = f'rung5_colonly_outlayer_r{args.rank}_s{args.seed}_{RUN_ID}'
_OUT   = _RUNS / _LABEL
...
def main():
    ...
    print('=' * 72)
    print(f'Rung 5 Color-Only Decoder LoRA  v4 (out_layer hook)')
    ...
    # try to reuse any existing slat cache from v1/v2/v3 runs
    _candidate_caches = [
        _HERE / 'runs' / 'rung5_colonly_r4_s6_5d6b1700'          / 'slat_cache.npz',  # v1
        _HERE / 'runs' / 'rung5_colonly_reg_r4_s6_b4f22c8e'      / 'slat_cache.npz',  # v2 (approx)
        # v3 run id is 0931a3d8
