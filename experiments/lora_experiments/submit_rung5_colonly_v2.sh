#!/bin/bash
# Submit rung5 color-only v2 (with L2 reg) decoder LoRA training.
# Usage: bash submit_rung5_colonly_v2.sh [--rank R] [--epochs E] [--seed S] [--lambda-reg L]

set -e

PY=/net/projects/ranalab/rajhansini/conda_envs/trellis/bin/python
SCRIPT=/net/projects/ranalab/rajhansini/TRELLIS/experiments/lora_experiments/rung5_colonly_lora_v2.py
...
echo "=== Rung 5 Color-Only Decoder LoRA v2 (with L2 reg) ==="
...
TRAIN_JOB=$(sbatch \
    ...
    --output="${LOGDIR}/rung5_colonyreg_%j.log" \
    --error="${LOGDIR}/rung5_colonyreg_%j.log" \
    --wrap="
set -e
...
echo "  tail -f ${LOGDIR}/rung5_colonyreg_${TRAIN_JOB}.log"
