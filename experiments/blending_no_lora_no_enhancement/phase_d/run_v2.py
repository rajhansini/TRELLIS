"""
Phase D — v2: per-position temporal attention, window [i-1, i, i+1], k=1.
...
def blend_v2(tok_prev, tok_curr, tok_next):
    ...
    return blended, attn.squeeze(1)                                    # (1374,1024), (1374,3)
...
def main():
    ...
    print('=== Phase D  v2  (per-position attention, window [i-1, i, i+1]) ===')
    ...
    for frame_i in range(start, start + n_frames):
        ...
        # ── WIRE: blend ───────────────────────────────────────────────────────
        K_hat, attn = blend_v2(tok_prev, tok_curr, tok_next)
        cond_gl     = K_hat.unsqueeze(0)
        attn_mean   = attn.mean(dim=0)                           # (3,) prev/curr/next
        # per position: which frame has max weight?
        argmax_pos  = attn.argmax(dim=1)                         # (1374,) 0=prev,1=curr,2=next
        pct_curr_wins = (argmax_pos == 1).float().mean().item() * 100

        attn_curr_log.append(attn_mean[1].item())

        print(f'\n=== frame {frame_i:04d}  (prev={frame_prev}, curr={frame_i}, next={frame_next}) ===')
        print(f'  [STEP2] tok_prev  shape={tuple(tok_prev.shape)}  mean={tok_prev.float().mean():.5f}  std={tok_prev.float().std():.5f}')
        ...
        print(f'  [ATTN]  mean: prev={attn_mean[0]:.4f}  curr={attn_mean[1]:.4f}  next={attn_mean[2]:.4f}  sum={attn_mean.sum():.4f}')
        print(f'  [ATTN]  curr has max weight at {pct_curr_wins:.1f}% of 1374 positions  {"✓ curr dominant" if pct_curr_wins > 50 else "✗ WARNING: curr not dominant"}')
        print(f'  [ATTN]  per-pos curr attn: min={attn[:,1].min():.4f}  max={attn[:,1].max():.4f}  std={attn[:,1].std():.4f}')
        ...
        del slat, cond_gl, K_hat, tok_prev, tok_curr, tok_next
    ...
    import numpy as np
    print(f'\n=== ATTENTION SUMMARY (phase_d v2, {len(attn_curr_log)} frames) ===')
    ...
    print(f'  frames where curr_attn > 0.33     : {sum(x>0.33 for x in attn_curr_log)}/{len(attn_curr_log)}')
    print(f'  (ideal: attn[curr] > 0.33 always, i.e. curr beats uniform 1/3 baseline)')
