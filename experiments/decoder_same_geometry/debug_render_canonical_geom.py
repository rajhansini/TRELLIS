"""
debug_render_canonical_geom.py
-------------------------------
Renders 150 frames with CANONICAL geometry + PER-FRAME LoRA colors.

Two-pass out_layer interception:
  Pass 1 (frozen): capture out_layer → canonical geometry channels
  Pass 2 (LoRA):   capture out_layer → LoRA color channels (COLOR_START:COLOR_END)
  Splice mixed features → to_representation() → clean mesh, no geometry holes.

Usage:
  python debug_render_canonical_geom.py --run-dir runs/rung5_54_r4_s6_31e5b3e8 \
...
def decode_both(feats, crds):
    """
    Returns (frozen_mesh, lora_canonical_geom_mesh).

    Two-pass out_layer interception:
      Pass 1: frozen decoder → save out_layer output → canonical geometry features
      Pass 2: LoRA decoder   → save out_layer output → LoRA color features
      Splice: canonical[:, :COLOR_START] + lora[:, COLOR_START:COLOR_END] + canonical[:, COLOR_END:]
      Pass mixed SparseTensor through dec_model.to_representation() → clean mesh.

    LoRA modifies ALL channels (SDF + deformation + color), causing geometry holes
    in thin parts (handle, lid, spout). This splice keeps frozen geometry and
    applies only LoRA color changes, giving a clean render.
    """
    ...
    # Splice: canonical geometry + LoRA colors only
    mixed_feats = frozen_h.feats.clone()
    mixed_feats[:, COLOR_START:COLOR_END] = lora_h.feats[:, COLOR_START:COLOR_END].to(mixed_feats.dtype)
