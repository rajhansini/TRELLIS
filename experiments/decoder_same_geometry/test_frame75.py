"""
Quick sanity: frame 75 only, 4-column comparison.
Loads SLaT from saved slat_cache.npz (generated by save_slat_cache.py).
Falls back to running the flow model if cache not found.
~2 min with cache, ~5 min without.
"""
...
OUT_IMG       = Path('/net/projects/ranalab/rajhansini/TRELLIS/experiments/decoder_same_geometry/frame075_4col.png')
FRAME_IDX     = 75
STRUCT_SEED   = 42
NOISE_SEED    = 6
...
# ── load SLaT: per-run cache → shared cache → flow model ─────────────────────
SHARED_CACHE = ROOT / 'experiments' / 'decoder_same_geometry' / 'rung5_slat_cache.npz'
SLAT_CACHE   = RUN_DIR / 'slat_cache.npz' if (RUN_DIR / 'slat_cache.npz').exists() else SHARED_CACHE
if SLAT_CACHE.exists():
    print(f'[SLAT] Loading from cache: {SLAT_CACHE}  (shared={SLAT_CACHE == SHARED_CACHE})')
    cache = np.load(SLAT_CACHE)
    slats_arr   = cache['slats']    # (150, N_vox, 8)
    coords_arr  = cache['coords']   # (N_vox, 4)  — already includes batch dim
    arr_idx = FRAME_IDX - 1
    ff75 = torch.from_numpy(slats_arr[arr_idx]).float()
    fc75 = torch.from_numpy(coords_arr).long()
    N_vox = ff75.shape[0]
    print(f'[SLAT] loaded  N_vox={N_vox}  feats={ff75.shape}')
    flow_model.cpu(); dino_model.cpu(); gc.collect(); torch.cuda.empty_cache()
else:
    print('[SLAT] No cache found — running flow model...')
    _dino_tfm = T.Compose([
    ...
    with torch.no_grad():
        feats_d = dino_model(t, is_training=True)['x_prenorm']
        toks    = F.layer_norm(feats_d, feats_d.shape[-1:])
    cond75 = toks
    ...
    print(f'[SLAT] Denoising frame {FRAME_IDX}...')
...
def _cap(mod, inp, out):
    canonical_feats['feats'] = out.feats.clone()
    canonical_feats['coords'] = out.coords.clone()
...
h.remove()
canonical_out_feats  = canonical_feats['feats']   # (N_up, 101)
canonical_out_coords = canonical_feats['coords']  # (N_up, 4)
print(f'[CANONICAL] feats={canonical_out_feats.shape}  coords={canonical_out_coords.shape}')

def _coord_key(coords):
    """Convert coords tensor to sort key: lexicographic on (batch,z,y,x)."""
    c = coords.long()
    return c[:, 0] * (10**12) + c[:, 1] * (10**8) + c[:, 2] * (10**4) + c[:, 3]

canon_keys = _coord_key(canonical_out_coords)
canon_sort_idx = torch.argsort(canon_keys)
...
print('[RENDER] LoRA direct (no geom swap)...')
if registry is not None:
    with torch.no_grad():
        with dec_lora_ctx(dec_model, registry):
            lora_direct_arr = render(filter_degen(dec_model(st75)[0]))
else:
    lora_direct_arr = frozen_arr.copy()

print('[RENDER] LoRA canonical geom swap (coord-aligned)...')
if registry is not None:
    def _hybrid(mod, inp, out):
        # Sort both by coords so row i matches the same spatial voxel
        lora_keys     = _coord_key(out.coords)
        lora_sort_idx = torch.argsort(lora_keys)
        lora_unsort   = torch.argsort(lora_sort_idx)  # inverse permutation

        canon_sorted = canonical_out_feats[canon_sort_idx]   # sorted canonical
        lora_sorted  = out.feats[lora_sort_idx]              # sorted lora

        hybrid_sorted = canon_sorted.clone().to(out.feats.dtype)
        hybrid_sorted[:, COLOR_START:COLOR_END] = lora_sorted[:, COLOR_START:COLOR_END]

        # Unsort back to lora's original ordering (needed for out.coords alignment)
        hybrid = hybrid_sorted[lora_unsort]
        return out.replace(hybrid)

    h = dec_model.out_layer.register_forward_hook(_hybrid)
    with torch.no_grad():
        with dec_lora_ctx(dec_model, registry):
            lora_canonical_arr = render(filter_degen(dec_model(st75)[0]))
    h.remove()
else:
    lora_canonical_arr = frozen_arr.copy()
...
LABEL_H = 28
canvas = Image.new('RGB', (W * 4, H + LABEL_H), (15, 15, 15))
draw   = ImageDraw.Draw(canvas)
for i, (arr, lbl, col) in enumerate([
    (gt_arr,             'GT video',          (80, 200, 80)),
    (frozen_arr,         'Frozen baseline',   (200, 80, 80)),
    (lora_direct_arr,    f'LoRA direct r{RANK}',    (80, 160, 255)),
    (lora_canonical_arr, f'LoRA canonical r{RANK}', (255, 200, 80)),
]):
