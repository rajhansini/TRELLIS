...
Uses Mesh decoder + MeshRenderer (nvdiffrast) — same renderer used during training,
so this is the fair evaluation of what the LoRA actually learned.

Loads the latest lora_e*.pt checkpoint from lora_ckpts/, runs the full pipeline
for every frame, saves rendered PNGs and a side-by-side comparison video.

Run from step8_train/:
  SPCONV_ALGO=native ATTN_BACKEND=xformers python infer_mesh.py
"""
...
OUT_DIR     = RESULTS_DIR / 'inference_frames_mesh'
...
# ── Camera: nvdiffrast convention — confirmed working in train.py ─────────────
# cam_x=(1,0,0), cam_y=(0,0,-1), cam_z=(0,1,0), t=2
# DO NOT change this — took 1 day to confirm correct orientation.
...
def main():
    ...
    # ── LoRA — load latest checkpoint from mesh-trained lora_ckpts/ ───────────
    ...
    # ── FrameWeightProjector (uniform λ) ──────────────────────────────────────
    ...
    for frame_num in range(1, N_FRAMES + 1):
        ...
        try:
            ...
            mask_t   = result['mask'].unsqueeze(0)
            color    = result['color'] * mask_t + (1.0 - mask_t)  # composite over white
    ...
    for prefix, label in [('render', 'render_mesh'), ('comparison', 'comparison_mesh')]:
        pattern = str(OUT_DIR / f'{prefix}_%04d.png')
        out_mp4 = str(RESULTS_DIR / f'{label}.mp4')
        cmd = ['ffmpeg', '-y', '-framerate', '24', '-i', pattern,
               '-c:v', 'mpeg4', '-q:v', '3', out_mp4]
        res = subprocess.run(cmd, capture_output=True)
        if res.returncode == 0:
            print(f'  Saved: {out_mp4}')
        else:
            print(f'  ffmpeg failed for {prefix}: {res.stderr.decode()[-200:]}')

    print('\nResults:')
    print(f'  Frames:         {OUT_DIR}/')
    print(f'  Render MP4:     {RESULTS_DIR}/render_mesh.mp4')
    print(f'  Comparison MP4: {RESULTS_DIR}/comparison_mesh.mp4')
