"""
Step 6b — Enhancement with FIXED NOISE SEED.

Identical to step06_enhancement.py EXCEPT the noise tensor is seeded with
the same fixed seed (base_seed=6) for EVERY frame, not base_seed+frame_i.

Purpose: isolate the contribution of noise consistency to temporal stability.
All frames start from the exact same random noise tensor → only the DINOv2
conditioning changes between frames.

Supports phase C/D smoothed tokens + matching alignment matrix, same as step06.
BACKWARD COMPATIBLE: default --phase raw = original behaviour.

Stages 1-4 (blending_no_lora_no_enhancement) are NOT touched.

Usage
-----
  # raw tokens (backward compatible)
  python step06b_fixed_noise.py --betas 0 1 2 3 4 6 8 16 --start_frame 1 --frames 150

  # phase C v1 C3
  python step06b_fixed_noise.py --phase c --variant v1 --config C3 --betas 0 1 2 3 4 6 8 16 --start_frame 1 --frames 150

  # phase D v2
  python step06b_fixed_noise.py --phase d --variant v2 --betas 0 1 2 3 4 6 8 16 --start_frame 1 --frames 150
"""
...
# pre-parse ALL args before Tee so log name encodes full experiment identity
...
_pre.add_argument('--frames',      type=int,   default=150)
...
_pre.add_argument('--asset',       type=str,   default='teapot_lava_kling_premium')
_pre.add_argument('--gt-dir',      type=str,   default='')
_pre.add_argument('--ref-frame',   type=int,   default=0,
                  help='structure reference frame; 0 = midpoint of --frames')
...
RESULTS   = _HERE / f'results_mcfm_{_PRE.asset}_{_exp_tag}'
RESULTS.mkdir(parents=True, exist_ok=True)

_LOG_NAME = (f'run_fixednoise_{_exp_tag}_betas{"_".join(str(b) for b in _PRE.betas)}'
...
GT_FRAMES_DIR = (Path(_PRE.gt_dir) if _PRE.gt_dir else
                 (_PIPE / '..' / '..' / '..' /
                  'MV-Adapter-Experimental/outputs/teapot_lava_kling_premium'
                  '/teapot_lava_kling_premium_front/all_frames_150'))
# Structure reference: the single frame whose geometry every frame is painted on.
# Midpoint by default, so a 121-frame asset uses 61 rather than a 75 that assumes 150.
_REF          = _PRE.ref_frame or ((_PRE.frames // 2) + 1)
GT_FRAME_75   = GT_FRAMES_DIR / f'frame_{_REF:04d}.png'

PRETRAINED  = 'microsoft/TRELLIS-image-large'
DEVICE      = torch.device('cuda')
STRUCT_SEED = 42
N_FRAMES    = _PRE.frames
...
def smooth_c_v1(tok_curr, tok_next, lc, ln):
    return lc * tok_curr + ln * tok_next
def smooth_c_v2(tok_curr, tok_next):
    Q = tok_curr.unsqueeze(1)
    K = torch.stack([tok_curr, tok_next], dim=1)
    attn = torch.softmax(torch.bmm(Q, K.transpose(1,2)) * _SCALE, dim=-1)
    return torch.bmm(attn, K).squeeze(1), attn.squeeze(1)
...
def smooth_c_v3(tok_curr, tok_next):
    # joint temporal+spatial: query the pooled bank, one softmax over time AND space
        ...
        def make_fwd(mod, nh_, hd_, sc_, bias, bt, blk_i, wc_log):
            ...
            def biased_fwd(x_st, context=None):
                ...
                if blk_i == 0 and wc_log is not None and not fired[0]:
                    fired[0] = True
                    with torch.no_grad():
                        attn_before = torch.softmax(scores_unbiased, dim=-1).mean(1)
                        attn_after  = torch.softmax(scores,          dim=-1).mean(1)
                        assigned    = bias.argmax(dim=1)
                        ab = attn_before[torch.arange(nv), assigned].mean().item()
                        aa = attn_after [torch.arange(nv), assigned].mean().item()
                        ab_rest = (attn_before.sum(1) - attn_before[torch.arange(nv), assigned]).mean().item() / 1373
                        aa_rest = (attn_after .sum(1) - attn_after [torch.arange(nv), assigned]).mean().item() / 1373
                        wc_log.append({
                            'before':     ab,
                            'after':      aa,
                            'rest_before': ab_rest,
...
def load_pipeline_and_coords():
    ...
    print(f'[LOAD] Sampling voxel structure (STRUCT_SEED={STRUCT_SEED}, frame 75)...')
...
def assemble_video(out_dir, start_frame=1):
    import subprocess
    subprocess.run([
...
def main():
    ...
    parser.add_argument('--asset',       type=str,   default='teapot_lava_kling_premium')
    ...
    fixed_seed = args.base_seed   # KEY: same seed for EVERY frame
    ...
    print('=' * 72)
    print('Step 6b — Fixed-Noise Enhancement  (noise seed fixed = same every frame)')
    print('=' * 72)
    print(f'  phase       : {phase}  variant={variant}  config={config or "n/a"}')
    print(f'  betas       : {betas}')
    print(f'  frames      : {start} .. {start + n_frames - 1}')
    print(f'  FIXED_SEED  : {fixed_seed}  ← same for ALL frames (vs step06: base_seed+frame_i)')
    ...
    # Verify fixed noise tensor
    torch.manual_seed(fixed_seed)
    _ref = torch.randn(N_vox, flow_model.in_channels)
    print(f'\n[FIXED NOISE] seed={fixed_seed}  shape={tuple(_ref.shape)}'
          f'  mean={_ref.mean():.6f}  std={_ref.std():.6f}  first5={_ref[0,:5].tolist()}')
    print(f'  Same tensor used for ALL {n_frames} frames.')
    del _ref

    # encode wider range for smoothing neighbors
    encode_start = max(1, start - (1 if phase == 'd' else 0))
    encode_end   = min(N_FRAMES, start + n_frames - 1 + (1 if phase in ('c','d') else 0))

    print(f'\n[ENCODE] Encoding raw DINOv2 tokens frames {encode_start}..{encode_end}...')
    ...
    print(f'\n[SMOOTH] phase={phase}  variant={variant}  config={config or "n/a"}')
    all_tokens = {}
    for i in range(start, start + n_frames):
        ...
        if phase == 'raw':
            ...
            smoothed = smooth_c_v1(tok_curr, raw_tokens[min(N_FRAMES,i+1)].to(DEVICE), cfg['lc'], cfg['ln'])
        elif phase == 'c' and variant == 'v2':
            smoothed, attn_w = smooth_c_v2(tok_curr, raw_tokens[min(N_FRAMES,i+1)].to(DEVICE))
            if i == start:
                aw = attn_w.mean(0)
                print(f'  [C v2 wire frame {i}] attn curr={aw[0]:.4f}  next={aw[1]:.4f}')
        elif phase == 'c' and variant == 'v3':
            smoothed, _ = smooth_c_v3(tok_curr, raw_tokens[min(N_FRAMES,i+1)].to(DEVICE))
        elif phase == 'd' and variant == 'v1':
            cfg = PHASE_D_V1[config]
            smoothed = smooth_d_v1(raw_tokens[max(1,i-1)].to(DEVICE), tok_curr,
                                   raw_tokens[min(N_FRAMES,i+1)].to(DEVICE),
                                   cfg['lp'], cfg['lc'], cfg['ln'])
        elif phase == 'd' and variant == 'v2':
            smoothed, attn_w = smooth_d_v2(raw_tokens[max(1,i-1)].to(DEVICE), tok_curr,
                                            raw_tokens[min(N_FRAMES,i+1)].to(DEVICE))
            if i == start:
                aw = attn_w.mean(0)
        ...
        if i == start:
            ...
            print(f'  [SMOOTH wire frame {i}] diff vs raw: mean={diff.mean():.5f}  '
                  ...
                  f'{"(zero = raw)" if diff.abs().max() < 1e-6 else "(non-zero = smoothing OK)"}')
    ...
    for beta in betas:
        ...
        if beta == 0.0:
            saved = None
            print(f'  beta=0 → original cross-attn  |  fixed_seed={fixed_seed}  phase={phase}')
        else:
        ...
        for frame_i in range(start, start + n_frames):
            cond_gl   = all_tokens[frame_i].to(DEVICE)
            if cond_gl.dim() == 2:
                cond_gl = cond_gl.unsqueeze(0)
            frame_num = frame_i - start + 1

            # KEY DIFFERENCE vs step06: same seed every frame
            torch.manual_seed(fixed_seed)
