#!/bin/bash
# Rebuild nvdiffrast + diff-gaussian-rasterization for a006 (RTX 2080 Ti, sm_75)
set -e
...
$PY -c "import torch; assert torch.cuda.is_available(), 'CUDA not available — run this inside your SLURM/srun GPU session'; p=torch.cuda.get_device_properties(0); print(f'GPU: {p.name}  sm_{p.major}{p.minor}  {p.total_memory//1024**3}GB')"

export TORCH_CUDA_ARCH_LIST="7.5"
echo "TORCH_CUDA_ARCH_LIST=${TORCH_CUDA_ARCH_LIST}"

BUILD_DIR=/tmp/renderer_build_a006_$$
mkdir -p "$BUILD_DIR"
echo "Build dir: $BUILD_DIR"

# ── 1. nvdiffrast ─────────────────────────────────────────────────────────────
echo ""
echo "=== Rebuilding nvdiffrast for sm_75 ==="
...
echo ""
echo "=== Rebuilding diff-gaussian-rasterization for sm_75 ==="
...
rm -rf "$BUILD_DIR"
echo ""
echo "=== Build complete for a006 (sm_75) ==="
