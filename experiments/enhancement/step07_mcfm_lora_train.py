"""
Step 7 — MCFM LoRA Training (v2_C / v2_D / v3_C / v3_D)

Continues from step06_mcfm_enhancement.py. Trains LoRA weights on the
flow model with MCFM blended tokens + frozen enhancement bias.

BACKWARD COMPATIBLE:
  - train_v2.py untouched (phase_c/d pipeline unaffected)
  - Same LoRA architecture: lora_v2 + dual_path_v2
  - Same artifact: enhancement/artifacts/voxel_to_token.pt (1748,)
  - Same fixed noise seed=6, STRUCT_SEED=42, LOSS_SCALE=4096

Architecture (dual-path cross-attention):
  PATH A (frozen) : q_base  @ K_hat^T  + beta*bias_map  → out_sp
  PATH B (LoRA)   : q_lora  @ K_pool^T                  → out_tp
  out_final       = to_out( out_sp + alpha * out_tp )

Usage:
  python step07_mcfm_lora_train.py --mode v2_C --beta 4.0
  python step07_mcfm_lora_train.py --mode v3_D --beta 4.0 --epochs 50
  python step07_mcfm_lora_train.py --mode v2_D --beta 4.0  # auto-resumes if ckpt exists
"""
...
_pre.add_argument('--beta',       type=float, default=4.0)
...
_RESULTS = _HERE / f'results_mcfm_{_PRE.mode}_lora_seed6'
_RESULTS.mkdir(parents=True, exist_ok=True)
_CKPT_DIR = _RESULTS / 'lora_ckpts'
_CKPT_DIR.mkdir(exist_ok=True)
_LOG_NAME = f'train_mode{_PRE.mode}_beta{_PRE.beta}_epochs{_PRE.epochs}.log'
...
ARTIFACT_PATH = _HERE / 'artifacts' / 'voxel_to_token.pt'
...
def blend(mode: str, raw_tokens: dict, frame_idx: int):
    ...
    # K_pooled = simple mean of window tokens (PATH B input for LoRA)
...
def denoise_last_step(flow_model, x_in, cond_gl, K_pooled,
                      lora_blocks, alpha, enhance_bias, require_grad: bool):
    t, t_prev = T_PAIRS[-1]
    t_ten = torch.tensor([1000.0 * t], device=DEVICE, dtype=torch.float32)
...
# ── Wire check — MCFM + enhancement + dual-path ───────────────────────────────

_SCALE = 1024 ** -0.5

def wire_check(mode: str, raw_tokens: dict, flow_model, enhance_bias,
               lora_blocks, alpha, coords, fixed_noise_feats, beta: float = 0.0):
    """
    Full startup wire check. Runs on frame 75. Verifies:
      1. MCFM blend is active (K_hat != raw token)
      2. K_pooled is a proper mean of window tokens
      3. Enhancement bias lifts attn@assigned in blk0
      4. LoRA PATH B fires (q_lora produces nonzero output at alpha=0.5)
      5. Dual-path output differs from frozen-only at alpha=0.5
    """
    print('\n[WIRE CHECK] Running on frame 75...')

    # ── 0. Step06 enhancement results exist ──────────────────────────────────
    beta_str  = str(int(beta)) + 'p0' if beta == int(beta) else f'{beta}'.replace('.', 'p')
    enh_dir   = _HERE / f'results_mcfm_{mode}_seed6_fixednoise' / f'beta{beta_str}'
    if enh_dir.exists():
        n_pngs  = len(list(enh_dir.glob('frame_*.png')))
        has_mp4 = (enh_dir / 'render.mp4').exists()
        print(f'  [W0-STEP06] {enh_dir.name}: n_frames={n_pngs}  render.mp4={has_mp4}  ✓')
    else:
        print(f'  [W0-STEP06] {enh_dir} NOT FOUND — step06 not yet run for mode={mode} beta={beta}')

    K_hat, K_pooled, win = blend(mode, raw_tokens, 75)
    tok_curr = raw_tokens[75].to(DEVICE)

    # ── 0b. cond_gl shape ────────────────────────────────────────────────────
    print(f'  [W0-COND  ] K_hat shape={tuple(K_hat.shape)}  '
          f'cond_gl shape={(1,)+tuple(K_hat.shape)}  '
          f'K_pooled shape={tuple(K_pooled.shape)}')

    # ── 1. MCFM blend ────────────────────────────────────────────────────────
    diff = (K_hat.float() - tok_curr.float()).abs()
    print(f'  [W1-MCFM] window={win}  mode={mode}')
    print(f'  [W1-MCFM] K_hat  mean={K_hat.float().mean():.5f}  std={K_hat.float().std():.5f}')
    print(f'  [W1-MCFM] K_hat vs tok[75]: diff_mean={diff.mean():.6f}  diff_max={diff.max():.6f}  '
          f'{"blending active ✓" if diff.max() > 1e-6 else "IDENTICAL ← no blend (degenerate)"}')
    ...
    print(f'  [W2-KPOOL] K_pooled shape={tuple(K_pooled.shape)}  '
          f'mean={K_pooled.float().mean():.5f}  std={K_pooled.float().std():.5f}  '
          f'pool_err={pool_err:.2e}  {"✓" if pool_err < 1e-5 else "MISMATCH ←"}')

    # ── 3. Enhancement bias effect on blk0 attn ──────────────────────────────
    blk0   = flow_model.blocks[0].cross_attn
    nh, hd = blk0.num_heads, blk0.channels // blk0.num_heads
    sc     = hd ** -0.5
    NVP    = enhance_bias.shape[0]
    # use first NVP rows of noise as proxy voxel feats
    torch.manual_seed(FIXED_SEED)
    proxy_q = torch.randn(NVP, nh, hd, device=DEVICE, dtype=torch.float32)
    proxy_k = K_hat[:NVP].unsqueeze(1).expand(-1, nh, -1)[..., :hd].float()
    scores_raw = torch.einsum('nhd,mhd->nhm', proxy_q, proxy_k) * sc  # (NVP, nh, NVP)
    scores_enh = scores_raw + enhance_bias.unsqueeze(1).float()
    ab = torch.softmax(scores_raw, dim=-1).mean(1)                   # (NVP, NVP)
    aa = torch.softmax(scores_enh, dim=-1).mean(1)
    asgn = enhance_bias.argmax(1)
    before = ab[torch.arange(NVP, device=DEVICE), asgn].mean().item()
    after  = aa[torch.arange(NVP, device=DEVICE), asgn].mean().item()
    ratio  = after / max(before, 1e-12)
    print(f'  [W3-BIAS ] NVP={NVP}  attn@assigned: before={before:.6f}  after={after:.6f}  '
          f'delta=+{after-before:.6f}  ratio={ratio:.1f}x  '
          f'{"✓" if ratio > 2.0 else "WEAK ← check beta"}')

    # ── 4. LoRA PATH B fires ─────────────────────────────────────────────────
    with torch.no_grad():
        blk_lora = lora_blocks[0]
        dummy_x  = torch.randn(NVP, blk0.channels, device=DEVICE, dtype=torch.float32)
        q_lora   = blk_lora.lora_q.forward(dummy_x.to(next(blk_lora.parameters()).dtype))
        kv_lora  = blk_lora.lora_kv.forward(
            K_pooled[:NVP].to(next(blk_lora.parameters()).dtype)
        )
    ...
    print(f'  [W4-LORA ] lora_q out max={q_norm:.4f}  lora_kv out max={kv_norm:.4f}  '
          f'alpha={alpha.item():.4f}  B_all_zero={B_zero}  '
          f'{"B=0 at init (expected) ✓" if B_zero else "B nonzero (resumed or bug) ←"}')

    # ── 5. Dual-path changes output vs frozen-only ───────────────────────────
    noise_sp = sp.SparseTensor(feats=fixed_noise_feats.clone(), coords=coords)
    cond_gl  = K_hat.unsqueeze(0)
    with torch.no_grad():
        for t, t_prev in T_PAIRS[:2]:   # just 2 steps — fast
            t_ten = torch.tensor([1000.0 * t], device=DEVICE, dtype=torch.float32)
            v_frozen = flow_model(noise_sp, t_ten, cond_gl)
            with dual_path_ctx_v2(flow_model, K_pooled, lora_blocks, alpha,
                                   enhance_bias=enhance_bias):
                v_dual = flow_model(noise_sp, t_ten, cond_gl)
            diff5 = (v_frozen.feats.float() - v_dual.feats.float()).abs()
            print(f'  [W5-DUAL ] step t={t:.3f}: frozen vs dual-path diff '
                  f'max={diff5.max():.6f}  mean={diff5.mean():.6f}  '
                  f'{"paths differ ✓" if diff5.max() > 1e-8 else "IDENTICAL ← dual-path not firing"}')
...
def gate1_backward_compat(flow_model, noise_sp, cond_gl, K_pooled,
                           lora_blocks, enhance_bias, coords):
    # Test LoRA plumbing in isolation (enhance_bias=None) so the bias doesn't
    # conflate results. With alpha=0 + B=zeros + no bias: both paths compute
    # q_base @ K_hat^T, so diff must be ~0.
    print('\n[GATE 1] Backward compat (alpha=0, B=zeros, enhance_bias=None — '
          'LoRA plumbing isolation, bias excluded from this test)...')
    ...
    def _run(use_lora):
        ...
        with torch.no_grad():
            for t, t_prev in T_PAIRS:
                t_ten = torch.tensor([1000.0 * t], device=DEVICE, dtype=torch.float32)
                if use_lora:
                    # enhance_bias=None: PATH A = q_base @ K_hat^T (no bias)
                    # alpha=0 + B=zeros: PATH B contributes 0
                    # → out_final == q_base @ K_hat^T == raw frozen
    ...
    # tol=1e-2: fp16 rounding from _attn_chunked (float32 explicit) vs xformers
    # native produces ~3e-3 diff — this is precision noise, not a real leak.
    # A genuine leak (e.g. alpha accidentally non-zero) produces diff >0.1.
    tol    = 1e-2
    print(f'  max_diff={diff.max():.6f}  mean_diff={diff.mean():.6f}  tol={tol}')
    if diff.max() < tol:
        print('  [GATE 1] PASSED ✓  (LoRA plumbing clean at alpha=0)')
    else:
        raise RuntimeError(f'GATE 1 FAILED: max_diff={diff.max():.6f} — LoRA plumbing leak at alpha=0')
...
def gate2_grad_check(lora_blocks, alpha):
    ...
    print(f'  alpha.grad: '
          f'{"NONE ← FAIL" if alpha.grad is None else f"{alpha.grad.item():.4f}"}')
    ...
    for vals, nm in [(q_A,'lora_q  A'), (q_B,'lora_q  B'), (kv_A,'lora_kv A'), (kv_B,'lora_kv B')]:
        if vals:
...
def main():
    ...
    print('=' * 72)
    print('Step 7 — MCFM LoRA Training')
    print('=' * 72)
    print(f'  mode        : {mode}')
    print(f'  beta        : {beta}  (frozen enhancement bias from step06)')
    print(f'  epochs      : {EPOCHS}')
    print(f'  lr          : {LR}')
    print(f'  lora_rank   : {LORA_RANK}')
    print(f'  loss_scale  : {LOSS_SCALE}  (fp16 FTZ fix)')
    print(f'  fixed_seed  : {FIXED_SEED}  (noise — all frames, all epochs)')
    ...
    if latest_ckpt:
        ...
        if 'mode' in ckpt and ckpt['mode'] != mode:
            raise RuntimeError(
                f'RESUME MISMATCH: checkpoint was trained with mode={ckpt["mode"]} '
                f'but current --mode={mode}. Delete {_CKPT_DIR} or use the correct --mode.')
        if 'beta' in ckpt and abs(ckpt['beta'] - beta) > 1e-6:
            raise RuntimeError(
                f'RESUME MISMATCH: checkpoint beta={ckpt["beta"]} != current --beta={beta}. '
                f'Use matching --beta or delete {_CKPT_DIR}.')
        print(f'  [RESUME-CHECK] mode={mode} ✓  beta={beta} ✓')
    ...
    if start_epoch > EPOCHS:
        print(f'All {EPOCHS} epochs done.')
        return

    # ── Artifact + enhancement bias ───────────────────────────────────────────
    print(f'\n[ALIGN] Loading {ARTIFACT_PATH}...')
    vox_to_tok = torch.load(ARTIFACT_PATH, weights_only=True).to(DEVICE)
    NVP        = vox_to_tok.shape[0]
    print(f'  shape={tuple(vox_to_tok.shape)}  range=[{int(vox_to_tok.min())}, {int(vox_to_tok.max())}]  '
          f'unique={vox_to_tok.unique().numel()}  (NVP={NVP} cross-attn voxels)')

    enhance_bias = build_soft_enhance_bias(vox_to_tok, beta, DEVICE)
    print(f'  enhance_bias shape={tuple(enhance_bias.shape)}  beta={beta}  PATH A only')

    # ── Fixed noise ───────────────────────────────────────────────────────────
    print(f'\n[NOISE] Building fixed noise (seed={FIXED_SEED})...')
    torch.manual_seed(FIXED_SEED)
    fixed_noise_feats = torch.randn(N_vox, flow_model.in_channels, device=DEVICE)
    print(f'  shape={tuple(fixed_noise_feats.shape)}  mean={fixed_noise_feats.mean():.6f}  '
          f'std={fixed_noise_feats.std():.6f}')
    print('  Same tensor cloned every frame every epoch.')
    ...
    print(f'  [SPOT-CHECK frame 75] mean={t75.mean():.5f}  std={t75.std():.5f}  '
          f'(expect ~0, ~1)')
    ...
    for fi in range(1, N_FRAMES + 1):
        ...
        if fi == 1:
            print(f'  frame 1: win={win}  K_pooled shape={tuple(kpooled_cache[fi].shape)}')
    ...
    gate1_backward_compat(flow_model, noise_sp_g1, cond_75, Kp_75,
                          lora_blocks, enhance_bias, coords)
    del noise_sp_g1, K_hat_75, Kp_75, cond_75

    # ── Wire check ────────────────────────────────────────────────────────────
    wire_check(mode, raw_tokens, flow_model, enhance_bias,
               lora_blocks, alpha, coords, fixed_noise_feats, beta=beta)
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        for frame_i in frame_list:
            ...
            K_pooled = K_pooled                 # (1374, 1024) — PATH B
            ...
            loss_val    = loss.item()
            epoch_loss += loss_val
            loss_history.append({'epoch': epoch, 'frame': frame_i,
    ...
    print(f'[DONE] Results: {_RESULTS}')
    print(f'[DONE] Log:     {_RESULTS / _LOG_NAME}')
