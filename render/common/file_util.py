...
def read_obj(file, texture=False):
    vs, normals, col, fs, txt= [], [], [], [], []
    f = open(file)
    for line in f:
        line = line.strip()
        ...
        if not splitted_line:
            ...
            fs.append([int(f.split('/')[0]) for f in splitted_line[1:4]])
        elif splitted_line[0] == 'vt':
            txt.append([float(n) for n in splitted_line[1:4]])
    f.close()
    vs = np.asarray(vs)
    col = np.asarray(col)
    normals = np.asarray(normals)
    faces = np.asarray(fs)
    if not texture and len(normals) > 0:
        if np.max(normals) > 1:
            normals = normals / 255
        points = np.concatenate((vs, normals), axis=1)
    else:
        points = vs
    return points, col, faces, txt
